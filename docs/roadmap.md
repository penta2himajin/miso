...
| M7c | K = 4096 GEMV (`ssm_out`, `attn_output`: 20 µs for 4.7 MB) and Q6_K qkv efficiency | Measured: shape-tuned R/grid in `gemv()` — K=4096 and large K=2048 use R=4; Q6_K qkv uses R=2 at grid 480. ssm_out 21.9 → 18.5 µs; decode 123.4 → **125.5 tok/s**. Then fused attn_q+attn_k (both Q4_K in every layer) on the 7 layers whose attn_v is Q6_K: 3 → 2 projection launches, decode → **126.0 tok/s**. A double-buffered Q6_K weight prefetch spilled (112 B scratch) and was reverted; microbenchmark-optimal R=5/R=6 and a smaller LM-head grid were each measured *slower* end to end and reverted |
